...
`/rv` 会向主会话注入**英文任务说明**。新的 Pi 审查会调用原生 `pi_review` 工具，由它以标准化事件模式启动打包的 `pi-review` CLI 并渲染实时 Panel。普通 `/rv @path` 无需额外参数；`--continue`、`--keep-session` 与显式 `--no-stream` 保持单审查 CLI 路径。
